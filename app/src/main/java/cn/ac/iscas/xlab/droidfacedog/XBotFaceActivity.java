    ...
    private void startTimerTask() {
        ...
        mDetectFaceTask = new TimerTask() {
            ...
            public void run() {
                ...
                if (face != null) {
                    Log.v(TAG, "Bitmap in mTextureView :" + face.getWidth() + "x" + face.getHeight() + ",Config:" + face.getConfig());
...
//                            Log.i(TAG, "mid pointF:" + mid.x + "," + mid.y);
        ...
        mDetectTimer.schedule(mDetectFaceTask, 500, 500);
    }
...
//            mPreviewSize = getPreferredPreviewSize(configMap.getOutputSizes(SurfaceTexture.class), width, height);
    ...
    public void speakOutUser(String userId) {
        ...
        text.append("欢迎参观中科院软件博物馆");
        String chineseName = Util.hexStringToString(userId);
        Log.i(TAG, "识别结果："+Util.hexStringToString(userId));
        if (userId == XBotFaceActivity.TTS_UNREGISTERED_USER) {
            mAudioManager.playAsync(11, new AudioManager.AudioCompletionCallback() {
                @Override
                public void onComplete(int id) {
                    mServiceProxy.publishAudioStatus(new AudioStatus(0,true));
                }
            });
        } else if (chineseName.equals("李松廷")) {
            mAudioManager.playAsync(12, new AudioManager.AudioCompletionCallback() {
                @Override
                public void onComplete(int id) {
                    mServiceProxy.publishAudioStatus(new AudioStatus(0,true));
                }
            ...
            mAudioManager.playAsync(13, new AudioManager.AudioCompletionCallback() {
                @Override
                public void onComplete(int id) {
                    mServiceProxy.publishAudioStatus(new AudioStatus(0,true));
                }
...
//                Log.i(TAG, "RecognizerListener -- onVolumeChanged()");
    ...
    //EventBus的回调，用来接收从Ros Server中发回来的机器人状态
...
//        boolean isMoving = status.isMoving();
        Log.i(TAG, "RobotStatus:" + status.toString());
            ...
            if (locationId == 0 ) {
                ...
                mAudioManager.playAsync(locationId - 1, new AudioManager.AudioCompletionCallback() {
                    @Override
                    public void onComplete(int id) {
                        mServiceProxy.publishAudioStatus(new AudioStatus(id, true));
                    }
                });
            }
    }
